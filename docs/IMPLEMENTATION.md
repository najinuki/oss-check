...
README와 LICENSE(Apache 2.0, DESIGN.md 1절)는 작성됐고 버전도 `0.1.0-SNAPSHOT`으로
...
- **릴리스 시 `-SNAPSHOT`을 떼야 한다.** 버전은 `build.gradle` 한 곳에서 jar 매니페스트로,
  거기서 `--version`과 덤프의 `toolVersion`으로 흘러간다(`ToolVersion`). 클래스로 직접
  실행하면 매니페스트가 없어 `"dev"`가 되고, 그런 덤프는 스스로 그렇다고 말한다.